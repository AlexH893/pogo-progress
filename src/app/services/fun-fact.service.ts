...
export class FunFactService {
  ...
  private readonly DISTANCE_MILESTONES: Milestone[] = [
    { threshold: 2.7, generate: (d) => `You've crossed the Golden Gate Bridge ${(d / 2.7).toLocaleString(undefined, { maximumFractionDigits: 0 })} times!` },
    ...
    { threshold: 8.8, generate: (d) => `You've walked enough to summit Mount Everest from sea level ${(d / 8.8).toLocaleString(undefined, { maximumFractionDigits: 1 })} times!` },
    { threshold: 11, generate: (d) => `You've walked the depth of the Mariana Trench ${(d / 11).toLocaleString(undefined, { maximumFractionDigits: 1 })} times!` },
    { threshold: 33, generate: (d) => `You've covered the width of the English Channel ${(d / 33).toLocaleString(undefined, { maximumFractionDigits: 1 })} times!` },
    ...
    { threshold: 344, generate: (d) => `You've walked the distance from London to Paris ${(d / 344).toLocaleString(undefined, { maximumFractionDigits: 1 })} times!` },
    { threshold: 400, generate: (d) => `You've walked enough to reach the International Space Station ${(d / 400).toLocaleString(undefined, { maximumFractionDigits: 1 })} times!` },
    { threshold: 898, generate: (d) => `You've walked the length of the Grand Canyon ${(d / 898).toLocaleString(undefined, { maximumFractionDigits: 1 })} times!` },
  ...
  private readonly POPULATION_MILESTONES: Milestone[] = [
    ...
    { threshold: 5000, generate: () => `Your lifetime catches outnumber the residents of a famous mountain town like Aspen, Colorado!` },
    { threshold: 10000, generate: () => `You've caught enough Pokémon to fully populate a scenic town like Sedona, Arizona!` },
    { threshold: 25000, generate: () => `Your total catches exceed the population of Key West, Florida!` },
    { threshold: 50000, generate: () => `If you kept every Pokémon you've caught, you'd have more residents than the country of Monaco!` },
    { threshold: 75000, generate: () => `You've caught enough Pokémon to match the population of Santa Fe, New Mexico!` },
    { threshold: 100000, generate: () => `Your total lifetime catches rival the population of a mid-sized city like Green Bay, Wisconsin!` },
    { threshold: 250000, generate: () => `Your lifetime catches rival the population of a major destination like Orlando, Florida!` },
  ...
  generateFacts(stats: ProfileStats | null, dailyAverages?: any): string[] {
    ...
    if (xp > 0) {
      if (xp < 20000000) {
        facts.push(`You are ${((xp / 20000000) * 100).toFixed(1)}% of the way to Level 40!`);
      } else if (xp < 176000000) {
        facts.push(`You are ${((xp / 176000000) * 100).toFixed(1)}% of the way to Level 50!`);
      ...
      facts.push(`You've earned enough XP to reach Level 40 over ${(xp / 20000000).toLocaleString(undefined, { maximumFractionDigits: 1 })} times!`);
    }
    ...
    if (dailyAverages) {
      if (dailyAverages.totalXp > 0) {
        if (xp < 176000000) {
          const daysTo50 = (176000000 - xp) / dailyAverages.totalXp;
